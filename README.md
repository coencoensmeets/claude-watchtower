...
**Paths in a message open.** A path written into the conversation is a place on this machine, so clicking one hands it to VS Code — a file at the line it was quoted at (`web/src/main.ts:1560`), a folder as a folder. The tool lines above a turn are the same, so the file an `Edit` touched is one click from the panel. What counts as a path is deliberately narrow, because every line of every message goes through it: something rooted (`/tmp/out.log`, `~/.motorcortex/build/…`, `./web`), or something with a folder and a suffix (`docs/cleanup-plan.md`) — and inside code marks, a bare file name with a suffix the panel knows (`server.py:44`). Which leaves `and/or`, `I/O`, `km/h`, `Array.from` and a URL alone. The panel will only open what already exists inside your home folder or the session's own, and says so when it will not; it is the same loopback-only route as the header's **Open in VS Code** button, and it goes away with the same setting.
...
| `POST /api/editor` | `{"sessionId": "...", "path": "...", "line": 12}` — open something in VS Code, if one of the `code` commands is on PATH. Without a path it is the session's own folder, read off the session. With one — how a path clicked out of a conversation is opened — the path must already exist and be inside your home folder or the session's own, or it is refused. Loopback only |
